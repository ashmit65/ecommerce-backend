...
async function createRecord(req, res) {
    try {
        ...
        console.log(error)
        const errorMessage = [];
        error.errors?.keyValue?.name? errorMessage.push({name:"Maincategory Already Exists"}):"", 
...
async function getSingleRecord(req, res) {
    try{
        // const data = await Maincategory.findOne().sort({_id:req.params._id});
        const data = await Maincategory.findById(req.params._id);
...
async function deleteRecord(req, res) {
    try{
        // const data = await Maincategory.findOne().sort({_id:req.params._id});
        // const data = await Maincategory.findById(req.params._id);
        const data = await Maincategory.findByIdAndDelete(req.params._id);
